...
allbrand_txt = open(os.path.join(script_dir, "allbrands.txt"), "r")
...
def ratio_digits(hostname):
    return len(re.sub(r"[^0-9]", "", hostname)) / len(hostname)
...
def random_domain(domain):
    try:
        ...
        nlp = nlp_class()
        return nlp.check_word(domain)
    except ModuleNotFoundError:
        return 0  # Default value if word_with_nlp.py is missing
